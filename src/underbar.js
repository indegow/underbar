(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    if (iterator) {
      ...
      var iteratedArray = [];
      result.push(array[0]);
      iteratedArray.push(iterator(array[0]));
  ...
  _.some = function(collection, iterator) {
    ...
    if (iterator === undefined) return !_.every(collection, function(a,b) {
      return (!a && !b);
    });
  ...
  _.memoize = function(func) {
    ...
    var result = [];
    return function() {
      for (var i = 0; i < alreadyCalled.length; i++) {
        if (JSON.stringify(arguments) === JSON.stringify(alreadyCalled[i])) {
          return result[i];
        }
      }
